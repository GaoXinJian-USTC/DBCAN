...
from DBCAN.models.builder import DECODERS, build_decoder
from DBCAN.models.textrecog.layers import CrossAttentionFusionLayer
from DBCAN.models.textrecog.layers.transformer_layer import PositionwiseFeedForward
from .base_decoder import BaseDecoder


@DECODERS.register_module()
class DBCANDecoder(BaseDecoder):
    ...
    def __init__(self,
        ...
        self.prediction = nn.Linear(dim_model if encode_value else dim_input,
                                    pred_num_classes)
        self.trans_dec = CrossAttentionFusionLayer(d_inner=2048)
    ...
    def forward_train(self, feat, out_enc, targets_dict, img_metas):
        ...
        position_glimpse = self.position_decoder.forward_train(
            feat, out_enc, targets_dict, img_metas)
        out = self.trans_dec(hybrid_glimpse,position_glimpse,position_glimpse)
    ...
    def forward_test(self, feat, out_enc, img_metas):
        ...
        for i in range(seq_len):
            ...
            hybrid_glimpse_step = hybrid_glimpse_step.permute(0,2,1)
            out = self.trans_dec(hybrid_glimpse_step,position_glimpse,position_glimpse)
            out = out[:,i,:]
            char_out = self.prediction(out)
