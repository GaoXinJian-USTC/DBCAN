...
class PositionalEmbedding2D(BaseModule):
    def __init__(self,
                 d_hid=512,
                 n_height=100,
                 n_width=100,
        ...
        self.embedding = nn.Embedding(self.max_seq_len + 1, dim_model)
        self.linear = nn.Linear(2*dim_model,dim_model)
        self.dropout = nn.Dropout(p=dropout)
    ...
    def forward(self, x):
        ...
        b, c, h, w = x.size()
        h_position_index = self._get_sinusoid_encoding_table(h,b)
        W_position_index = self._get_sinusoid_encoding_table(w,b)
        
        
        emd = torch.cat([h_position_index,W_position_index],1)

        emd_pos = self.linear(emd)

        out = self.dropout(emd_pos)

        return out
    
# fun = PositionalEmbedding2D()

# x = torch.rand((2,512,6,40))
# print(fun(x).shape)
